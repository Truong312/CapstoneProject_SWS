    ...
    public class InventoryRepository: GenericRepository<Inventory>, IInventoryRepository
    {
        ...
        public async Task<List<ProductInventoryDto>> GetAllProductInventoryAsync()
        {
         ...
         .Select(p => new ProductInventoryDto(
             p.ProductId,
             p.Name,
             _context.Inventories.Where(i => i.ProductId == p.ProductId).Sum(i => i.QuantityAvailable),
             _context.Inventories.Where(i => i.ProductId == p.ProductId && i.Status == InventoryStatus.Available).Sum(i => i.QuantityAvailable),
             _context.Inventories.Where(i => i.ProductId == p.ProductId && i.Status == InventoryStatus.Allocated).Sum(i => i.QuantityAvailable),
             _context.Inventories.Where(i => i.ProductId == p.ProductId && i.Status == InventoryStatus.Damaged).Sum(i => i.QuantityAvailable),
             _context.Inventories.Where(i => i.ProductId == p.ProductId && i.Status == InventoryStatus.InTransit).Sum(i => i.QuantityAvailable)
         ))
         .ToListAsync();
